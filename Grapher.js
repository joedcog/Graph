var Graph = function(aobj) {
      ...
      if (isInt(z)) {
        ...
        if (z % 0.5 === 0) {
          ...
          for (var i = 0; i < size * (b - a); i++) {
            if (xVal - (1 / size) < a) {
              tempY = ((evaluateGammaEquation(equation, xVal) + evaluateGammaEquation(equation, a)) * .5 * (1 / size));
            } else {
              tempY = ((evaluateGammaEquation(equation, xVal) + evaluateGammaEquation(equation, xVal - (1 / size))) * .5 * (1 / size));
            }
            if (i > 0) {
              if (!isFinite(tempY)) {
                break;
  ...
  this.drawGraphAxis = function() {
    ...
    var box = plot.setAttribute('viewBox', '0 0 ' + this.imageWidth + ' ' + this.imageHeight);
